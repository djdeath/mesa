...
#define I965_BATCH_EMIT(cmd, name)                                      \
   for (struct cmd name = { __i965_cmd_header(cmd) },                   \
           *_dst = anv_batch_emit_dwords(batch, __i965_cmd_length(cmd)); \
        __builtin_expect(_dst != NULL, 1);                              \
        ({ __i965_cmd_pack(cmd)(batch, _dst, &name);                    \
           VG(VALGRIND_CHECK_MEM_IS_DEFINED(_dst, __i965_cmd_length(cmd) * 4)); \
           _dst = NULL;                                                 \
        }))

#define I965_RELOC(buf, r_domains, w_domain, dt)        \
   (struct i965_address) {                              \
      .buffer = buf,                                    \
      .offset = (__map - brw->batch.map) * 4,           \
      .read_domains = r_domains,                        \
      .write_domain = w_domain,                         \
      .delta = dt,                                      \
   }
